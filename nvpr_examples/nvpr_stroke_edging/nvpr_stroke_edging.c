...
/* nvpr_stroke_edging.c - NV_path_rendering of internal and external edging */

// Demonstrate how to use NV_path_rendering to stroke the "internal edging" of a stroke (the portion of
// the stroke region that is also in the fill region) and the "external edging" of a stroking (the
// portion of the stroke region that is NOT in the fill region).
...
const char *program_name = "nvpr_stroke_edging";
GLuint pathObj = 42;
int path_specification_mode = 0;
int filling = 1;
int stroking = 1;
int internal_edging = 1;
int external_edging = 1;
int even_odd = 0;
...
void initGraphics()
{
    ...
    glPathParameterfNV(pathObj, GL_PATH_STROKE_WIDTH_NV, 16.5);
}
...
doGraphics(void)
{
    ...
    if (internal_edging) {
        if (even_odd) {
            glStencilFillPathNV(pathObj, GL_COUNT_UP_NV, 0x1);
        } else {
            glStencilFillPathNV(pathObj, GL_COUNT_UP_NV, 0x7F);
        }
        glPathStencilFuncNV(GL_NOTEQUAL, 0, ~0);
        glStencilStrokePathNV(pathObj, 0x80, 0x80);
        glPathStencilFuncNV(GL_ALWAYS, 0, ~0);
        ...
        // Disable color writes so we can cover and reset all remaining non-zero stencil values to zero.
        glColorMask(0,0,0,0);
        glStencilFunc(GL_NOTEQUAL, 0, ~0);
        glCoverFillPathNV(pathObj, GL_CONVEX_HULL_NV);
        glColorMask(1,1,1,1);
    }
    ...
    if (external_edging) {
        ...
        glPathStencilFuncNV(GL_EQUAL, 0, ~0);
        glStencilStrokePathNV(pathObj, 0x80, 0x80);
        glPathStencilFuncNV(GL_ALWAYS, 0, ~0);

        glStencilFunc(GL_EQUAL, 0x80, 0x80);
        glColor3f(173/255.0f, 216255.0f, 230255.0f); // light blue
        glStencilOp(GL_KEEP, GL_KEEP, GL_ZERO);
        glCoverStrokePathNV(pathObj, GL_CONVEX_HULL_NV);
...
keyboard(unsigned char c, int x, int y)
{
    ...
    case 's':
        ...
        break;
    case 'i':
        internal_edging = !internal_edging;
        printf("internal_edging = %d\n", internal_edging);
        break;
    case 'x':
        external_edging = !external_edging;
        printf("external_edging = %d\n", external_edging);
        break;
    default:
        return;
    }
