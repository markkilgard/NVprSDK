// Noise GLSL shader for path rendering
...
// Uniforms
layout(location = 0) uniform sampler1D lut;

void main()
{
    float noise_factor = cnoise(scale*position/vec2(3142,3142)+offset);
    vec4 red = vec4(0.8,0.2,0.1,1);
    vec4 blue = vec4(0.1,0.5,0.9,1);
    color = mix(red, blue, noise_factor);
}
