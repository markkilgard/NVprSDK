...
layout(location = 8) uniform vec4 loColor;
layout(location = 9) uniform vec4 hiColor;
...
void main()
{
    ...
    color = 1-texture(lut,snoise(scale*position/vec2(3142,3142)+offset));
   
    float nn = cnoise(position);
    color = mix(loColor,hiColor,nn);
    //color = vec4(position,0,1);
}
