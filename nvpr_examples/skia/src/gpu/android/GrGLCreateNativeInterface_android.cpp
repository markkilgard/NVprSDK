...
{
    // glPolygonMode isn't in OpenGL ES. (If it's really needed, we can set
    // some state here and then insert line loops during the draw calls.)
}
...
const GrGLInterface* GrGLCreateNativeInterface() {
    ...
    if (!glInterface.get()) {
        ...
        interface->fPolygonMode = glPolygonMode;
        ...
        if (GrGLHasExtensionFromString("GL_ARB_texture_storage", extensions))
            interface->fTexStorage2D = reinterpret_cast<GrGLTexStorage2DProc>(eglGetProcAddress("glTexStorage2D"));
        else if (GrGLHasExtensionFromString("GL_EXT_texture_storage", extensions))
            interface->fTexStorage2D = reinterpret_cast<GrGLTexStorage2DProc>(eglGetProcAddress("glTexStorage2DEXT"));
#endif
